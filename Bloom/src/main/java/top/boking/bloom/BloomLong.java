 ...
 * 采用long型数组存储元素
 */
public class BloomLong implements Bloom{
    ...
    /**
     * memory size get in properties file
     * so,this get Method not need parms
     * 双锁
     *
     * @return 布隆long单例对象
     */
    public static BloomLong getBloomLong() {
        if (bloomLong == null) {
            synchronized (BloomLong.class) {
                if (bloomLong == null) {
                    bloomLong = new BloomLong();
                    return bloomLong;
                }
            }
        }
    ...
    private static BloomLong bloomLong;
     ...
     * @return
     */
    public void insertFast(String key) {
        for (int i = 0; i <= K_USEHASHCODENUMBER; i++) {
            Integer integer = HashCodeLib.choiceHashMethod(key, i);
            logger.trace(i);
            insert(integer);
     ...
     * @return
     */
    public boolean insertKeySecurity(String key) {
...
//              return  1==((MEMORY[(-1 >>> 32 - MOVE << 6 & i ) >>> 6] >>> (i & 63))&1);
     ...
     * @return 理论所需空间（bit）
     */
    private static long getNeedMemorySize(long n, double p) {
     ...
     * @return 实际分配的内存空间（位）
     */
    private static long getRealMemorySize(long n, double p) {
     ...
     * @return
     */
    private static int getHashCodeMethodNumber(long n, long m) {
     ...
     * @return 真实失误率
     */
    private static double getRealMissProbability(long n, long m, int k) {
        float f = n * k;
        float e = f / m;
