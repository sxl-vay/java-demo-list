...
public class ModeInfo {
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        ModeInfo testStater = new ModeInfo();
        ...
        valueMap.forEach((k, v) -> {
            ...
            if (k.equalsIgnoreCase("modeinfo")) {
                v.forEach(m ->{
                    String cubeuuid = m.get("cubeuuid");
                    String modename = m.get("modename");
                    String space = "";
                    for (int i = 0; i < 50 - cubeuuid.length(); i++) {
                        space+=" ";
                    }
                    System.out.println(String.format("cubeuuid:%s%s modename:%s", cubeuuid,space, modename));
                });
            }
            /*if (k.equalsIgnoreCase("workflow_bill")) {
                v.forEach(m ->{
                    String tablename = m.get("id");
